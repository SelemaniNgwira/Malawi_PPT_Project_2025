...
## Table 23 Receive or report health information using mobile phone or app
...
ppt_data |> 
       select(demo_v, rece_repo_healthinfo) |> 
       tbl_summary(by = rece_repo_healthinfo,
              missing = "no", 
  ...
  modify_caption("**Receive or report health information using mobile phone or app**") %>%
   add_ci() %>%
   add_p() %>%
  modify_fmt_fun(
...
# prefered methods of reporting . 

```{r}
ppt_data |> 
       select(demo_v, rece_repo_alert) |> 
       filter(!is.na(rece_repo_alert)) %>%
             tbl_summary(by = rece_repo_alert,
              missing = "no", 
              label = list( gender~ "Gender",
                           age_group2~ "Age group",
                           edu~ "Education level",
       ...
       statistic = list(
          ...
          all_continuous() ~ "{mean} ± {sd}" )  )%>%
  modify_header(label = "**Variable**") %>%
  modify_caption("**Preferred Methods of Reporting health information**") %>%
   add_ci() %>%
   add_p() %>%
  modify_fmt_fun(
    statistic = label_style_percent(symbol = FALSE),
...
#| echo: false
 ...
 ppt_data %>%
  select(all_of(demo_v), starts_with("motivate_healthsur_")) |> 
   select(-motivate_healthsur_specify) |>
   pivot_longer(
     cols = starts_with("motivate_healthsur_"),
     names_to = "motivation",
     values_to = "response") |> 
   filter(response==1) |> 
   select(-response) |> 
   mutate(motivation=recode(motivation, 
                            "motivate_healthsur_result"="Knowing results",
                            "motivate_healthsur_helpcom"="Helping community",
                            "motivate_healthsur_trustmoh"="Trust in MoH",
                            "motivate_healthsur_incentive"="Incentives",
                            "motivate_healthsur_advice"="Receiving advice",
                            "motivate_healthsur_easysystem"="Easy system",
                            "motivate_healthsur_other"= "Other")) |> 
   tbl_summary(
     by = motivation,
     missing = "no", 
    ...
    modify_caption("**Motivation factors to participate in digital health surveillance**"
  ) %>%
   add_ci() %>%
   add_p() 
...
#| echo: false
 ...
 ppt_data %>%
  select(all_of(demo_v), starts_with("stop_healthsur_")) |> 
   select(-c(stop_healthsur_specify_71, stop_healthsur_specify_72)) |> 
   pivot_longer(
     cols = starts_with("stop_healthsur_"),
     names_to = "barrier",
     values_to = "response") |> 
   filter(response==1) |> 
   select(-response) |> 
   mutate(barrier=recode(barrier, 
                         "stop_healthsur_poornet"= "Poor network",
                         "stop_healthsur_nonelec" = "Not confident with phones",
                         "stop_healthsur_nofeedback"= "No feedback", 
                         "stop_healthsur_privacy"= "Privacy concerns",
                         "stop_healthsur_other"= "Other")) |> 
 tbl_summary(
     by = barrier,
     missing = "no", 
    ...
    modify_caption("*Barriers to participate in digital health surveillance*") %>%
   add_ci() %>%
   add_p()
